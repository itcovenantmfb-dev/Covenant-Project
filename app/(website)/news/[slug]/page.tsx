...
const RelatedPostCard: React.FC<{ post: NewsArticle }> = ({ post }) => (
...
export default async function BlogPostPage({
  ...
  // Get all news articles and filter out current one for related posts
  const allNews = await getAllNews();
  const relatedPosts = allNews.filter((p: NewsArticle) => p._id !== post._id).slice(0, 4);
        ...
        <section className="bg-[#F9FFF5] py-20 sm:py-24 w-full">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {relatedPosts.map((relatedPost: NewsArticle) => (
...
export async function generateStaticParams() {
  ...
  return posts.map((post: NewsArticle) => ({
